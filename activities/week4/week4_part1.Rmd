...
num_obs = Orange %>% group_by(Tree) %>% tally()
...
```{r q2}
Orange_1 = Orange %>% transform(age = age/365, circumference = circumference/10)
...
Orange %>% group_by(age) %>% 
  ...
  mutate(zscore = (circumference-mean)/sd) %>% 
  select(-c(mean,sd))
```
...
```{r q4}
orange_r = Orange_1 %>% mutate(avg_rate = circumference/age) 
...
It returns a 7x3 dataframe with each row represents the average rate(mm/day) of growth across trees and the standard deviation of the average rate for  a given age(day)
